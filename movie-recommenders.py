...
class MovieRecommender:
    ...
    def _create_user_item_matrix(self):
        ...
        # Ensure movieId columns are sorted the same as movies_df for later alignment
        # (optional but helps keep things predictable)
        self.user_item_matrix = self.user_item_matrix.reindex(
            columns=sorted(self.user_item_matrix.columns), fill_value=0
        )
    ...
    def _train_collaborative_filtering(self):
        ...
        # USER-BASED CF: use count of ratings to select active users
        print("   👥 Computing user similarities...")
        user_rating_counts = (self.user_item_matrix > 0).sum(axis=1)
        active_users = user_rating_counts[user_rating_counts >= self.min_ratings].index
        if len(active_users) == 0:
            # if no active users with that threshold, lower the threshold
            active_users = user_rating_counts.nlargest(5).index

        active_user_matrix = self.user_item_matrix.loc[active_users]

        # Mean-center each user's ratings (only over non-zero ratings), then fill NaN with 0
        active_user_matrix_masked = active_user_matrix.replace(0, np.nan)
        user_means = active_user_matrix_masked.mean(axis=1)
        user_centered = active_user_matrix_masked.sub(user_means, axis=0).fillna(0)

        self.user_similarity = pd.DataFrame(
            cosine_similarity(user_centered.values),
            index=active_user_matrix.index,
            columns=active_user_matrix.index
        )

        # ITEM-BASED CF: select active movies by count of ratings
        print("   🎬 Computing item similarities...")
        movie_rating_counts = (self.user_item_matrix > 0).sum(axis=0)
        active_movies = movie_rating_counts[movie_rating_counts >= self.min_ratings].index
        if len(active_movies) == 0:
            active_movies = movie_rating_counts.nlargest(10).index

        active_movie_matrix = self.user_item_matrix[active_movies].T  # movies x users

        # Mean-center each movie's ratings (only over non-zero ratings), then fill NaN with 0
        active_movie_matrix_masked = active_movie_matrix.replace(0, np.nan)
        movie_means = active_movie_matrix_masked.mean(axis=1)
        movie_centered = active_movie_matrix_masked.sub(movie_means, axis=0).fillna(0)

        self.item_similarity = pd.DataFrame(
            cosine_similarity(movie_centered.values),
    ...
    def _train_content_based(self):
        ...
        # Compute content similarity matrix (rows in item_features correspond to movies_df order)
        # Align movieIds: assume movies_df order corresponds to TF-IDF rows
        movie_ids = list(self.movies_df['movieId'].values)
    ...
    def _train_matrix_factorization(self):
        ...
        # Prepare data for SVD (use full user-item matrix with zeros for missing)
        svd_matrix = self.user_item_matrix.values

        # Apply SVD
        self.svd_model = TruncatedSVD(
            n_components=min(self.n_factors, min(svd_matrix.shape)-1),
        ...
        explained = getattr(self.svd_model, "explained_variance_ratio_", None)
        explained_sum = explained.sum() if explained is not None else np.nan

        print(f"   ✅ SVD model trained with {self.n_factors} factors")
        if not np.isnan(explained_sum):
            print(f"   ✅ Explained variance ratio: {explained_sum:.3f}")
    ...
    def recommend_user_based(self, user_id, n_recommendations=10):
        ...
        # If the user isn't in the user_similarity (not active), fall back to popular
        if user_id not in self.user_similarity.index:
            return self._get_popular_movies(n_recommendations)

        # Find similar users (excluding the user itself if present)
        user_similarities = self.user_similarity.loc[user_id].drop(user_id, errors='ignore')
        top_similar_users = user_similarities.nlargest(20)
        ...
        for similar_user, similarity in top_similar_users.items():
            ...
            for movie_id, rating in rated_by_similar.items():
                if movie_id not in user_movies:
                    recommendations.setdefault(movie_id, 0.0)
        ...
        for movie_id, rating in user_movies.items():
            ...
            for similar_movie, similarity in similar_movies.items():
                if (similar_movie != movie_id and
                        ...
                        similarity > 0):
                    recommendations.setdefault(similar_movie, 0.0)
    ...
    def recommend_hybrid(self, user_id, n_recommendations=10,
        ...
        for movie_id in all_movies:
            ...
            if movie_id in user_recs:
                score += weights.get('user_based', 0) * user_recs[movie_id]
            if movie_id in item_recs:
                score += weights.get('item_based', 0) * item_recs[movie_id]
            if movie_id in content_recs:
                score += weights.get('content_based', 0) * content_recs[movie_id]
    ...
    def _get_popular_movies(self, n_movies=10):
        ...
        return [(int(idx), float(row['score'])) for idx, row in top_movies.iterrows()]
    ...
    def find_similar_movies(self, movie_id, n_similar=5, method='item_based'):
        ...
        if method == 'item_based' and movie_id in self.item_similarity.index:
            ...
            similar_movies = similarities.drop(movie_id, errors='ignore').nlargest(n_similar)
        ...
        return [(int(idx), float(score)) for idx, score in similar_movies.items()]
    ...
    def get_user_profile(self, user_id):
        ...
        profile = {
            ...
            'avg_rating': float(user_ratings['rating'].mean()) if not user_ratings.empty else None,
            'rating_std': float(user_ratings['rating'].std()) if not user_ratings.empty else None,
...
def create_sample_data():
    ...
    for user_id in range(1, 11):  # 10 users
        ...
        movie_ids = np.random.choice(list(movies_data['movieId']), n_ratings, replace=False)
